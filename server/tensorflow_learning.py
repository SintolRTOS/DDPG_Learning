# -*- coding: utf-8 -*-
"""
Created on Mon May 27 15:55:13 2019
...
# # 使用新的构建的图
graph = tf.Graph()
#此时在这个代码块中，使用的就是新定义的图graph()(相当于把默认的图换成graph())
...
#会还构建&启动(默认情况下（不给定Session的graph参数的情况下），创建的Session属于默认的图)
sess = tf.Session()
#print(sess)
# # 调用sess的run方法来执行矩阵的乘法，得到c的结果值（所以将c作为参数传递进去）
# # 不需要 考虑图中间的运算，在运行的时候只需要关注最终结果对印尼你给的对象以及所需要的输入数据值
# # 只需要传递进去所需要得到的结果对象，会自动的根据图中的依赖关系触发所有相关的欧赔操作的执行。
...
with tf.Session(config=tf.ConfigProto(log_device_placement=True,allow_soft_placement=True)) as sess:
    ...
    sess.run(init_op)
# # init_op.run()#这行代码也是初始化运行操作，但是要求明确给定当前代码块对应的默认
#session(tf.get_default_session())是那个，底层使用默认session来运行
# #获取操作的结果
...
# #1.定义变量，常量
w1 = tf.Variable(tf.random_normal(shape=[10],stddev=0.5,seed=28,dtype=tf.float32))
w2 = tf.Variable(w1.initialized_value()*a,name='w2')
a = tf.constant(value=2.0,dtype=tf.float32)
print("-------------------------------------------------------------")
print(w1)
# # 3.进行初始化操作(推荐：使用全局所以变量初始化API)
# 相当于在图中加入一个初始化全局变量的操作
